//! Custom Win32 "download failed, try again?" dialog" — same
//! mockup-aligned layout as [`error_window`] /
//! [`metadata_failed_window`], with **two buttons** (Try again
//! + Cancel) and an error info-icon.
//!
//! Layout (640×320):
//!
//! ```text
//! ┌───────────────────────────────────────────────────────────────┐
//! │ [icon] Download failed — Try again?            — □ ✕       │
//! │                                                                │
//! │  ┌──────────────┐  Heading (large bold)                       │
//! │  │              │  Subheading (grey)                           │
//! │  │   [mascot]   │                                               │
...
//! │  └──────────────┘                                               │
//! │  ┌──────────────────────────────────────┐  ┌────────────┐       │
//! │  │ ⚠  Why did this happen?              │  │ Try again  │       │
//! │  │    See the message above for…          │  │   Cancel   │       │
//! │  └──────────────────────────────────────┘  └────────────┘       │
//! └───────────────────────────────────────────────────────────────┘
//! ```
//!
//! Two buttons at the right of the info box — primary "Try
//! again" on the left, secondary "Cancel" on the right. Enter
//! activates the primary; Esc dismisses via the secondary (X also
//! dismisses via secondary — `WM_CLOSE` in the WndProc).
//!
//! **TODO:** `error_window`, `metadata_failed_window`, and this
//! module are nearly identical (same WM_CREATE / WM_PAINT /
//! WM_CTLCOLORSTATIC / WM_COMMAND / / WM_CLOSE / WM_NCDESTROY
//! handlers, same helpers, same layout). The only per-dialog
//! differences are the strings and (for the error / info icon) the
//! `IDI_*` id. A future refactor should collapse all three into a
//! shared `modal_window::show(...)` that takes the heading /
//! subheading / info-heading / info-subtext / content + button
//! labels + icon id, and one WndProc handles all of them.
...
use windows_sys::Win32::UI::WindowsAndMessaging::{
    ...
    GetSystemMetrics, GetWindowLongPtrW, IDCANCEL, IDI_ERROR, LoadIconW, PostQuitMessage,
    RegisterClassExW, SendMessageW, SetWindowLongPtrW, TranslateMessage, BS_DEFPUSHBUTTON,
    BS_PUSHBUTTON, MSG, SM_CXSCREEN, SM_CYSCREEN, WM_CLOSE, WM_COMMAND, WM_CREATE,
    WM_CTLCOLORSTATIC, WM_NCDESTROY, WM_PAINT, WNDCLASSEXW, WS_CAPTION, WS_CHILD,
...
const CLASS_NAME: &str = "snug_retry_dialog_v1\0";
...
const BUTTON_PRIMARY_W: i32 = 100;
...
const COLOR_SUBTITLE: i32 = 0x005F6368;
...
///
/// `IDYES_I32` (6) = primary action (Try again). `IDCANCEL_I32`
/// (2) = secondary action. The launcher retries the install on
/// `IDYES_I32` and gives up on `IDCANCEL_I32`.
...
pub unsafe fn show(parent: HWND, dlg: RetryDialog<'_>) -> i32 {
    ...
    let info_heading_text: String = dlg
        ...
        .filter(|s| !s.is_empty())
        .unwrap_or_else(|| {
            let from_toml = dialogs.jdk_install.retry.info_heading.clone();
    ...
    let info_subtext_text: String = dlg
        ...
        .filter(|s| !s.is_empty())
        .unwrap_or_else(|| {
            let from_toml = dialogs.jdk_install.retry.info_subtext.clone();
            if from_toml.is_empty() {
    ...
    let heading_text = if dlg.heading.is_empty() {
        dialogs.jdk_install.retry.heading.clone()
    } else {
        dlg.heading.to_string()
    };
    let subheading_text = if dlg.subheading.is_empty() {
        dialogs.jdk_install.retry.subheading.clone()
    } else {
    ...
    if hwnd.is_null() {
        log::log("retry_window: CreateWindowExW returned NULL — aborting");
...
const INFO_SUBTEXT_DEFAULT: &str =
    "A common cause is a transient network blip. You can retry, or cancel and try again later.";
...
static CLASS_AT: OnceLock<u16> = OnceLock::new();

fn register_class() -> u16 {
    *CLASS_AT.get_or_init(|| unsafe {
...
const IDC_HEADING: i32 = 4001;
const IDC_SUBTITLE: i32 = 4002;
const IDC_CONTENT: i32 = 4003;
const IDC_INFO_ICON: i32 = 4004;
const IDC_INFO_HEADING: i32 = 4005;
const IDC_INFO_SUBTEXT: i32 = 4006;
const IDC_PRIMARY: i32 = 4007;
const IDC_SECONDARY: i32 = 4008;
...
unsafe extern "system" fn r_wndproc(
    hwnd: HWND,
    msg: u32,
    wparam: WPARAM,
    lparam: LPARAM,
    ...
    match msg {
        WM_CREATE => unsafe {
            ...
            let state = (*create_struct).lpCreateParams as *mut RState;
            ...
            // Error icon (red ❌) — distinct from
            // `metadata_failed_window`'s warning ⚠ because retry is
            // about a transient failure, not a permanent one.
            let icon_handle = LoadIconW(std::ptr::null_mut(), IDI_ERROR as *const u16);
        ...
        WM_PAINT => unsafe {
            ...
            let raw = GetWindowLongPtrW(hwnd, GWLP_USERDATA) as *mut RState;
            ...
            if mascot_hbitmap != 0 {
                draw_mascot(hdc, mascot_hbitmap as _);
                ...
                find_best_icon_hicon(MASCOT_LOAD_CX, MASCOT_LOAD_CY)
            {
                ...
                {
                    log::log("retry_window WM_PAINT: DrawIconEx failed");
                }
            } else {
                log::log("retry_window WM_PAINT: no usable EXE icon found");
            }
        ...
        WM_CTLCOLORSTATIC => unsafe {
            ...
            if !raw.is_null() && hwnd_child != (*raw).hwnd_info_icon {
                ...
                if grey_text {
                    SetTextColor(hdc, COLOR_SUBTITLE as u32);
        ...
        WM_CLOSE => {
            // X / Alt+F4: treat as secondary action so the dialog
            // actually quits instead of leaving the message loop
            // spinning forever (DefWindowProcW only DestroyWindow).
            unsafe {
...
/// Same drawing routine as `error_window` / `metadata_failed_window`.
/// Duplicated rather than shared so each module is self-contained.
unsafe fn draw_mascot(hdc: HDC, hbitmap: isize) {
    use windows_sys::Win32::Graphics::Gdi::{
    ...
    if bm_ok == 0 {
        log::log("retry_window mascot: GetObjectW failed");
        return;
    }
