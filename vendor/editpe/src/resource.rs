...
impl ResourceDirectory {
    ...
    /// Store arbitrary bytes under a custom `RT_RCDATA` (type `0x0A`)
    /// resource entry named `name`, at language `en-US` (`0x0409`).
    ///
    /// Mirrors [`Self::set_manifest`] but for opaque binary blobs the
    /// Windows runtime tools don't have a typed API for. Use this to
    /// attach data the launcher reads at startup — for example, an
    /// embedded application payload.
    ///
    /// `name` is stored as a Unicode resource entry name (so it can be
    /// inspected by tools like Resource Hacker). Repeated calls with
    /// the same name replace the previous data.
    ///
    /// # Returns
    /// Returns an error if the resource table structure is not well-formed.
    pub fn set_rcdata(&mut self, name: &str, data: Vec<u8>) -> Result<(), ResourceError> {
        let rcdata_table_name = ResourceEntryName::ID(RT_RCDATA as u32);
        let mut rcdata_table = self
            .root
            .cloned_table_or_default(&rcdata_table_name, "rcdata table is not a table")?;
        ...
        self.root
            .insert(rcdata_table_name, ResourceEntry::Table(rcdata_table));

        Ok(())
    }

    /// Retrieve raw bytes previously stored under a custom `RT_RCDATA`
    /// resource entry named `name` at language `en-US`.
    ///
    /// Returns `Ok(None)` if no such entry exists.
    ///
    /// # Returns
    /// Returns an error if the resource table structure is not well-formed.
    pub fn get_rcdata(&self, name: &str) -> Result<Option<Vec<u8>>, ResourceError> {
        if self.root.entries.is_empty() {
            return Ok(None);
        }

        let rcdata_table = match self.root.get(ResourceEntryName::ID(RT_RCDATA as u32)) {
            Some(ResourceEntry::Table(t)) => t,
            Some(_) => {
                return Err(ResourceError::InvalidTable(
                    "rcdata table is not a table".to_string(),
                ));
            }
            None => return Ok(None),
        };
        let inner_table = match rcdata_table.get(ResourceEntryName::from_string(name)) {
            Some(ResourceEntry::Table(t)) => t,
            Some(_) => {
                return Err(ResourceError::InvalidTable(
                    "inner rcdata table is not a table".to_string(),
                ));
            }
            None => return Ok(None),
        };
        let data_entry = match inner_table.get(ResourceEntryName::ID(LANGUAGE_ID_EN_US as u32)) {
            Some(ResourceEntry::Data(d)) => d,
            Some(_) => {
                return Err(ResourceError::InvalidTable(
                    "rcdata entry is not data".to_string(),
                ));
            }
            None => return Ok(None),
        };

        Ok(Some(data_entry.data().to_vec()))
    }
